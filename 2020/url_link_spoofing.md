...
脆弱性を報告した2019年1月当時、SlackのUIはハイパーリンクを投稿できない仕様であった。しかし、APIにはハイパーリンクを投稿するための `<{url}|{text}>` という記法が存在する<sup id="f1">[1](#fn1)</sup>。そこで、投稿時のHTTPリクエストを以下のように操作すると、偽装したURLリンクを投稿できた。
...
Slackのワークスペースでは同じ名前のメンバーが共存できる<sup id="f2">[2](#fn2)</sup>。そのため、ワークスペース内で影響力のある人物になりすまして偽装したURLリンクを投稿することで、他のメンバーからのクリック率を高められると考えた。レポートでは、FacebookのCEOであるMark Zuckerberg氏が参加するワークスペースを仮定し、求人募集をうたう投稿の例を示した。

<img src="/assets/2020/url_link_spoofing/fake_url.png" width="800" alt="fake_url">

Slackは当初、「この攻撃はワークスペース内でのメンバー間の対立が前提になっている」と述べ、URLリンクの偽装を脆弱性として認めなかった。その後、私はSlackとOutlookを連携できることを知り<sup id="f3">[3](#fn3)</sup>、もしメンバーのひとりがOutlookで受信したメールをSlackに転送していた場合、攻撃者はそのメンバー宛にメールを送り付けることで、外部から偽装したURLリンクを投稿できると考えた。「そのような攻撃はメンバー間の対立とは限らない」と再検討を依頼すると、Slackはこの挙動を脆弱性として対策を講じた。
...
2020年2月にSlackは、UIからハイパーリンクを投稿できる機能をリリースした<sup id="f4">[4](#fn4)</sup>。書式設定のリンクアイコン `🔗` から任意のリンクを生成できるため、実際のリンク先と表示が異なるURLリンクの投稿も可能である。しかし、そのように偽装されたURLリンクをクリックすると、警告ダイアログが表示されるようになった。
...
Slackに限らず、サイトやアプリの至る所にあるURLリンクを私は日々クリックし、タップしている。偽装の事例を知って以来、アクセスする前に実際のリンク先を気にするようになった。主要なブラウザやメーラーでは、リンクにカーソルを合わせることでステータスバーからURLを確認できる。モバイルアプリも多くの場合、リンクの長押しによりURLを確認できる。ただし、JavaScriptによる遷移先までは確認できないため、攻撃者の意図したスクリプトが動作する場合は、ステータスバーの表示も偽装されている可能性がある<sup id="f5">[5](#fn5)</sup>。信用できない環境ではリンクを踏まないという判断も必要だ。

---

<sup id="fn1">[1](#f1)</sup> <a href="https://api.slack.com/reference/surfaces/formatting#linking-urls" onclick="this.href='https://evil.akaki.io'">https://api.slack.com/reference/surfaces/formatting#linking-urls</a>  
<sup id="fn2">[2](#f2)</sup> <a href="https://twitter.com/buritica/status/970721576034455552" onclick="this.href='https://evil.akaki.io'">https://twitter.com/buritica/status/970721576034455552</a>  
<sup id="fn3">[3](#f3)</sup> <a href="https://slackhq.com/increase-everyday-productivity-with-office-365-apps-for-slack" onclick="this.href='https://evil.akaki.io'">https://slackhq.com/increase-everyday-productivity-with-office-365-apps-for-slack</a>  
<sup id="fn4">[4](#f4)</sup> <a href="https://twitter.com/SlackHQ/status/1230587660948762624" onclick="this.href='https://evil.akaki.io'">https://twitter.com/SlackHQ/status/1230587660948762624</a>  
<sup id="fn5">[5](#f5)</sup> https://www.owenboswarva.com/URLspoof.htm  
